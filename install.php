...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  ...
  if (file_exists($envFile)) {
    ...
    jsonResponse(['error' => 'Instalación previa detectada. Eliminá api/ y reinstall.php primero.', 'step' => 2, 'log' => $log], 409);
  }
  ...
  $includePaths = [
    ...
    'api',            // PHP backend
    'migration',      // SQL files
    'images',         // Static images (if in repo)
    'logos',          // SVG logos
    '.htaccess',
    'hero-video.mp4',
  ];
  ...
  foreach ($includePaths as $path) {
    $src = $repoDir . '/' . $path;
    if (!file_exists($src)) {
      addLog("$path (no encontrado en repo, saltando)", false);
      continue;
    }
    ...
    if (is_dir($src)) {
      ...
      addLog("$path/ (" . iterator_count($it) . " archivos)");
    } else {
      ...
      $totalFiles++;
      $size = filesize($src);
      addLog("$path (" . ($size > 1024 ? number_format($size / 1024, 1) . ' KB' : $size . ' B') . ")");
    }
  }
