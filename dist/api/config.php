...
if (file_exists($envFile)) {
  ...
  foreach ($lines as $line) {
    ...
    $key = trim($key);
    $value = trim($value, " \t\n\r\0\x0B\"'");
    // Set via putenv, $_ENV, and $GLOBALS as fallback (putenv may be disabled)
    if (!getenv($key)) {
      @putenv("$key=$value");
    }
    $_ENV[$key] = $value;
    $GLOBALS['_ENV_' . $key] = $value;
  }
}

define('DB_HOST', getenv('DB_HOST') ?: ($_ENV['DB_HOST'] ?? $GLOBALS['_ENV_DB_HOST'] ?? 'localhost'));
define('DB_NAME', getenv('DB_NAME') ?: ($_ENV['DB_NAME'] ?? $GLOBALS['_ENV_DB_NAME'] ?? 'oteguiobra_web'));
define('DB_USER', getenv('DB_USER') ?: ($_ENV['DB_USER'] ?? $GLOBALS['_ENV_DB_USER'] ?? 'oteguiobra_web'));
define('DB_PASS', getenv('DB_PASS') ?: ($_ENV['DB_PASS'] ?? $GLOBALS['_ENV_DB_PASS'] ?? ''));
